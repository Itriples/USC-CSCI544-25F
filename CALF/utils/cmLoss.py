...
class cmLoss(nn.Module):
    def __init__(self, feature_loss, output_loss, task_loss, task_name,
                 feature_w=0.01, output_w=1.0, task_w=1.0):
    ...
    def forward(self, outputs, batch_y, in_sample=None, freq_map=None, batch_y_mark=None):
        # Always-present time branch
        outputs_time = outputs["outputs_time"]
        intermidiate_feat_time = outputs["intermidiate_time"]

        # Optional text branch (for original CALF). For NCL-only CALF this will
        # fall back to using the time branch, avoiding KeyError.
        outputs_text = outputs.get("outputs_text", outputs_time)
        intermidiate_feat_text = outputs.get("intermidiate_text", intermidiate_feat_time)
        ...
        if self.feature_w != 0:
            ...
            feature_loss = torch.tensor(0.0, device=outputs_time.device, dtype=outputs_time.dtype)
        ...
        if self.output_w != 0:
            if self.task_name == "long_term_forecast":
                ...
                output_loss = torch.tensor(0.0, device=outputs_time.device, dtype=outputs_time.dtype)
        else:
            output_loss = torch.tensor(0.0, device=outputs_time.device, dtype=outputs_time.dtype)
        ...
        if self.task_name == "long_term_forecast":
            ...
            task_loss = torch.tensor(0.0, device=outputs_time.device, dtype=outputs_time.dtype)

        total_loss = (
            self.task_w * task_loss
            + self.output_w * output_loss
            + self.feature_w * feature_loss
        )
        return total_loss
